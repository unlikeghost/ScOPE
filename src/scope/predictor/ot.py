...
class ScOPEOT(_BasePredictor):
    ...
    def _reshape_data(data: np.ndarray) -> np.ndarray:
        batch_size, n_samples, compressors, metrics = data.shape
        
        return data.reshape(batch_size, n_samples, compressors * metrics).squeeze(0)
    
    def __forward__(self, current_cluster: np.ndarray, current_sample: np.ndarray) -> float:
              
        if self.aggregation_method is not None:
            current_cluster = self._reshape_data(current_cluster)
            current_sample = self._reshape_data(current_sample)
